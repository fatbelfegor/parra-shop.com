...
(function() {
  this.roomInit = function(editor) {
    var attachToGrid, combinedBold, combinedThin, config, createGrid, drawLine, room;
    combinedThin = new THREE.Geometry();
    combinedBold = new THREE.Geometry();
    drawLine = function(line, combined) {
      var geometry;
    ...
    createGrid = function(size) {
      var combined, drawSquare, newSize, room;
      drawSquare = function(line, cb1, cb2, combined) {
        line = drawLine(line, combined);
        line = cb1(line);
        line = drawLine(line, combined);
        line = cb2(line);
        line = drawLine(line, combined);
        return drawLine(cb1(line), combined);
      };
      newSize = {
        width: size.width,
        height: size.height,
      ...
      room.id = 'room';
      room.add(new THREE.Line(combinedThin, new THREE.LineBasicMaterial({
        color: size.color1
      }, THREE.LineStrip)));
      room.add(new THREE.Line(combinedBold, new THREE.LineBasicMaterial({
        color: size.color2
      }, THREE.LineStrip)));
      combinedThin = new THREE.Geometry();
      combinedBold = new THREE.Geometry();
