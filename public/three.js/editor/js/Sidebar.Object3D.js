Sidebar.Object3D = function ( editor ) {
	...
	var getRoom = function () {
		return new Config().getKey( 'room' )
	}

	container.add(new UI.Button().setLabel( 'Передвинуть влево' ).onClick(function () {
		editor.selected.position.x = -getRoom().width/2 - editor.selected.geometry.boundingBox.min.x
	}))
	container.add(new UI.Button().setLabel( 'Передвинуть вправо' ).onClick(function () {
		editor.selected.position.x = getRoom().width/2 - editor.selected.geometry.boundingBox.max.x
	}))
	container.add(new UI.Button().setLabel( 'Передвинуть вперед' ).onClick(function () {
		editor.selected.position.z = getRoom().depth/2 - editor.selected.geometry.boundingBox.max.z
	}))
	container.add(new UI.Button().setLabel( 'Передвинуть назад' ).onClick(function () {
		editor.selected.position.z = -getRoom().depth/2 - editor.selected.geometry.boundingBox.min.z
	}))
	container.add(new UI.Button().setLabel( 'Передвинуть вверх' ).onClick(function () {
		editor.selected.position.y = getRoom().height/2 - editor.selected.geometry.boundingBox.max.z
	}))
	container.add(new UI.Button().setLabel( 'Передвинуть вниз' ).onClick(function () {
		editor.selected.position.y = -getRoom().height/2 - editor.selected.geometry.boundingBox.min.z
	}))
