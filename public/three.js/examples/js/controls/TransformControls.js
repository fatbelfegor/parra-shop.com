...
(function () {
	...
	THREE.TransformControls = function ( camera, domElement ) {
		...
		function onPointerDown( event ) {
			...
			this.room = this.config.getKey( 'room' )
			this.attachToGrid = this.config.getKey( 'attachToGrid' )
		...
		function onPointerMove( event ) {
			...
			if ( _mode == "translate" ) {
				...
				if ( scope.space == "world" || scope.axis.search("XYZ") != -1 ) {
					...
					if (this.attachToGrid) {
						point.x = Math.round(point.x/10)*10
						point.y = Math.round(point.y/10)*10
						point.z = Math.round(point.z/10)*10
					}
					var difference = scope.object.geometry.boundingBox.max.y + oldPosition.y
					var max = this.room.height/2
					if (difference + point.y >= max) {
						point.y = max - difference;
					} else {
						difference = scope.object.geometry.boundingBox.min.y + oldPosition.y
						if (difference + point.y <= -max) {
							point.y = -max - difference;
						}
					}
					difference = scope.object.geometry.boundingBox.max.x + oldPosition.x
					max = this.room.width/2
					if (difference + point.x >= max) {
						point.x = max - difference;
					} else {
						difference = scope.object.geometry.boundingBox.min.x + oldPosition.x
						if (difference + point.x <= -max) {
							point.x = -max - difference;
						}
					}
					difference = scope.object.geometry.boundingBox.max.z + oldPosition.z
					max = this.room.width/2
					if (difference + point.z >= max) {
						point.z = max - difference;
					} else {
						difference = scope.object.geometry.boundingBox.min.z + oldPosition.z
						if (difference + point.z <= -max) {
							point.z = -max - difference;
						}
					}
