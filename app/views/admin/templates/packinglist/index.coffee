					...
					if items.length
						post "packinglist/create", {pack: pack, items: items}, (d) ->
							...
							ret += "'>#{pack.date}</td>
								...
								<td>#{amount}</td>
								<td><a class='btn blue' href='/admin/packinglist/#{pack.id}' onclick='app.click(this)'>Открыть</a></td>
	...
	destroy: (el, id) ->
		...
		ask "Удалить запись?",
			...
			action: ->
				...
				id = tr.data 'id'
				$.ajax
					url: "/admin/model/packinglist/destroy/#{id}"
					type: 'POST'
					contentType: false
					processData: false
					dataType: "json"
					success: (res) ->
						if res is 'permission denied'
							...
							tr.remove()
							delete models['packinglist'].collection[id]
...
app.page = ->
	...
	<div class='content'>
		<label class='text-center'><div class='btn blue ib m15'>Выбрать файл накладной (HTML)</div><div class='hide'></div><input class='hide' onchange='functions.upload(this)' type='file' multiple></label>
